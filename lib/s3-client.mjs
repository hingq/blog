...
export class S3Client {
  ...
  async send(command) {
    ...
    const method = command.name === 'PutObjectCommand' ? 'PUT' : 'GET'
    const now = new Date()
    ...
    const canonicalUri = requestPath.split('/').map(segment => encodeURIComponent(segment)).join('/')
    ...
    const signingKey = getSignatureKey(this.credentials.secretAccessKey, dateStamp, this.region, service)
    ...
    if (!response.ok) {
      ...
      const error = new Error(`S3 operation failed: ${response.status} ${response.statusText}\n${errorText}`)
